...
export class Knob implements OnInit {
  ...
  drawArcs(clickInteraction, dragBehavior) {
    ...
    if (this.options.displayInput) {
      var fontSize = (this.options.size * 0.25) + "px";
      ...
      if (this.options.subText.enabled) {
        fontSize = (this.options.size * 0.1) + "px";
        ...
        svg.append('text')
          ...
          .attr('transform', 'translate(' + ((this.options.size / 2)) + ', ' + ((this.options.size / 2) + (this.options.size * 0.3)) + ')');
      }
    }
    if (this.options.scale.enabled) {
